 ...
 *演示select的用法， 这是一个代理软件的核心，专门做端口映射
 *copy：zhoulifa 老师
 */
...
static connect_socket(int connect_port, char *address)
{
  struct sockaddr_in a;
  int s;
  if((s = socket(AF_INET, SOCK_STREAM, 0)) < 0){
...
int main(int argc, char *argv[])
{
  ...
  int buf1_avail, buf1_writen;
  int buf2_avail, buf2_writen;
  ...
  for( ; ; ){
    ...
    /*把监听socket和可读的三个一起放入select的可读句柄列表里*/
    nfds = max(nfds, h);
    ...
    /*把可写socket两个一起放入select的可写的句柄列表中*/
    if(fd1 > 0 && buf2_avail - buf2_writen > 0){
      FD_SET(fd1, &wr);
      nfds = max(nfds, fd1);
    }
    if(fd2 > 0 && buf1_avail - buf1_writen > 0){
      FD_SET(fd2, &wr);
      nfds = max(nfds, fd2);
    }

    /*把有异常数据的socket两个一起放入select的异常句柄列表中*/
    if(fd1 > 0){
      FD_SET(fd1, &er);
      nfds = max(nfds, fd1);
    }
